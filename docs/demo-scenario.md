...
Dans la section `Commandes du sujet`, choisir un onglet (`keylog`, `rdp`, `crack`, `pth`, `loot`, `phish`, `propagate`, `privesc`, `syscall`, `shell` ou `whatever`) puis cliquer sur `Tester la commande sélectionnée`. Le tableau de bord ajoute une alerte MITRE et une entrée d'audit, mais n'exécute aucune action Windows réelle.
